...
export default function AdminPage() {
    ...
    const openEdit = (dish) => { setForm({ ...dish }); setEditing(dish.id); setModal(true); };
                    ...
                    {dishes.map((dish) => (
                        <div key={dish.id} className="card p-4 flex gap-3">
                            ...
                            <div className="flex flex-col gap-1">
                                ...
                                <button onClick={() => handleDelete(dish.id)} className="p-1.5 rounded-lg hover:bg-red-50 dark:hover:bg-red-950/30 text-red-500 transition-colors"><Trash2 size={14} /></button>
                            </div>
                        </div>
                    ))}
                </div>
